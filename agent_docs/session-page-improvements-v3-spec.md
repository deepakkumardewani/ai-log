...
5. **Tool rows (unified format)** — one shared pattern: `● <Tool> <primary arg>`. The body
   reveals IN and/or OUT on click; **a section is shown only when that data exists** in the
   log (no empty IN/OUT). Dot is green.
6. **Skill** — row shows `<full-skill-name> skill` (e.g. `agent-skills:interview-me skill`).
   Clicking the name opens a **modal** with the full skill body.
7. **Read** — row shows file name + line range. Clicking the filename opens the **file
   contents (the tool OUT) in a modal**.
...
10. **Images** — small **horizontally-stacked thumbnails**. Clicking a thumbnail opens it
    **full-size in a modal** with a close button.
